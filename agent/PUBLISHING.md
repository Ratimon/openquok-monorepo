...
Match `agent/package.json` `version` to the tag (`cli-v0.0.15` → `"0.0.16"`). Commit to `main`, then tag **that** commit:

```bash
git add agent/
git commit -m "chore(cli): release 0.0.16"
git push origin main
git tag cli-v0.0.16
git push origin cli-v0.0.16
```
