...
**Plan (to implement when ready):** deploy **two always-on services** from the **orchestrator** package build (`orchestrator/dist/`), each with its own start command, sharing production env and Redis with the API.
...
**Production start commands** (after `pnpm --filter openquok-common build && pnpm --filter backend build && pnpm --filter openquok-orchestrator build`, run from `orchestrator/` or with `cwd` set to `orchestrator`):

```bash
node dist/worker/runIntegrationRefreshBullMqWorker.js
node dist/worker/runNotificationEmailBullMqWorker.js
```
