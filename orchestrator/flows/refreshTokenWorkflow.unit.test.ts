 ...
 *
 * **Transport:** Orchestrator Jest clears `ORCHESTRATOR_*_TRANSPORT` after loading env so this suite stays on
 * `in_process` (see `orchestrator/jest.orchestrator-default-transport.cjs`). BullMQ enqueue behavior is covered in
 * `refreshTokenWorkflow.bullmq.unit.test.ts` (`pnpm test:unit:refresh-token-workflow:bullmq`).
 */
