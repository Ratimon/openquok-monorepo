...
function getEnvTrimmed(key, defaultValue = "") {
...
function maxMediaUploadBytesForMime(mimetype, surface) {
  ...
  return MAX_MEDIA_IMAGE_UPLOAD_BYTES_BACKEND;
}
...
function validateMediaFileUploadSize(size, mimetype, surface) {
  if (!Number.isFinite(size) || size < 0) {
    return "Invalid file size.";
  }
  const max = maxMediaUploadBytesForMime(mimetype);
  if (size <= max) {
    return null;
  }
...
function isPaidSubscriptionTier(tier) {
  return PAID_SUBSCRIPTION_TIERS.includes(tier);
}
...
function normalizeMediaVirtualPath(path7) {
  ...
  if (!raw || raw === "/")
    return MEDIA_VIRTUAL_GENERAL;
  const withLead = raw.startsWith("/") ? raw : `/${raw}`;
  return withLead.replace(/\/+$/, "") || MEDIA_VIRTUAL_GENERAL;
}
function resolveMediaVirtualPath(path7) {
  return normalizeMediaVirtualPath(path7);
}
function mediaFileManagerId(virtualPath, mediaId, displayName) {
...
function parseMediaFileManagerId(id) {
...
function isProtectedMediaVirtualFolder(path7) {
  const normalized = normalizeMediaVirtualPath(path7);
  return MEDIA_VIRTUAL_PROTECTED_FOLDERS.includes(normalized);
}
function mediaVirtualPathFromFileManagerTarget(targetId) {
  const parsed = parseMediaFileManagerId(targetId);
  if (parsed)
    return parsed.virtualPath;
  return normalizeMediaVirtualPath(targetId);
}
function sanitizeMediaDisplayName(name) {
  const base = String(name ?? "file").trim().replace(/[/\\?%*:|"<>]/g, "-").replace(/\s+/g, " ").slice(0, 180);
...
function configuredStripePriceId(priceIds, tier, period) {
  const row = priceIds[tier];
  const id = period === "MONTHLY" ? row.monthly : row.yearly;
  const trimmed = id?.trim();
  return trimmed ? trimmed : null;
}
var init_stripePriceConfig = __esm({
  "config/stripePriceConfig.ts"() {
    init_dist();
    init_envHelper();
  }
...
function orchestrationTransportFromEnv(envKey, fallback) {
...
var init_GlobalConfig = __esm({
  "config/GlobalConfig.ts"() {
    ...
    ({ loadBackendDotenv } = loadBackendDotenvCjs);
...
var supabaseConfig, serverConfig, supabaseAnonSingleton, supabase, createSupabaseRLSClient;
var init_supabase = __esm({
  "connections/supabase.ts"() {
    ...
    createSupabaseRLSClient = ({ req, res }) => {
      const cookies = {
        getAll() {
...
var CacheError, CacheConnectionError, CacheOperationError;
...
var DEFAULT_TTL_SEC, CacheService, CacheService_default;
var init_CacheService = __esm({
  "connections/cache/CacheService.ts"() {
    ...
    CacheService_default = CacheService;
  }
});

// connections/cache/CacheInvalidationService.ts
var CacheInvalidationService, CacheInvalidationService_default;
var init_CacheInvalidationService = __esm({
  "connections/cache/CacheInvalidationService.ts"() {
    init_Logger();
    ...
    CacheInvalidationService_default = CacheInvalidationService;
  }
});

// connections/cache/MemoryCacheProvider.ts
var MemoryCacheProvider, MemoryCacheProvider_default;
var init_MemoryCacheProvider = __esm({
  "connections/cache/MemoryCacheProvider.ts"() {
    init_Logger();
    ...
    MemoryCacheProvider_default = MemoryCacheProvider;
  }
});
var RedisCacheProvider, RedisCacheProvider_default;
var init_RedisCacheProvider = __esm({
  "connections/cache/RedisCacheProvider.ts"() {
    ...
    RedisCacheProvider_default = RedisCacheProvider;
  }
});
...
var cacheConfig, cacheService, cacheInvalidationService;
var init_cache = __esm({
  "connections/cache/index.ts"() {
    init_GlobalConfig();
    init_Logger();
    init_CacheService();
    init_CacheInvalidationService();
    init_MemoryCacheProvider();
    init_RedisCacheProvider();
    cacheConfig = config.cache;
    cacheService = new CacheService_default(createCacheProvider(), {
    ...
    cacheInvalidationService = new CacheInvalidationService_default(cacheService);
  }
});
var sentryConfig, serverConfig2, dsn, sentryEnabled;
var init_sentry = __esm({
  "connections/sentry/index.ts"() {
    init_GlobalConfig();
    sentryConfig = config.sentry;
    serverConfig2 = config.server;
    dsn = (sentryConfig?.dsn ?? "").toString().trim();
    sentryEnabled = sentryConfig?.enabled !== false;
    if (dsn && sentryEnabled && !Sentry__namespace.isInitialized()) {
      ...
      Sentry__namespace.init({
        dsn,
        sendDefaultPii: true,
        environment: (serverConfig2?.nodeEnv ?? "development").toString(),
        ...
        beforeSend(event, hint) {
          if (serverConfig2?.nodeEnv === "development") {
            const msg = event.message ?? (hint.originalException instanceof Error ? hint.originalException.message : "");
...
var init_metrics = __esm({
  "connections/sentry/metrics.ts"() {
    init_sentry();
    METRIC_NAME = "public_api-request";
  }
});
...
// connections/bullmq/index.ts
var init_bullmq = __esm({
  "connections/bullmq/index.ts"() {
    init_createQueueIoredis();
  }
});
function isR2ConnectionReady(cfg) {
  return Boolean(
    cfg.accountId?.trim() && cfg.accessKeyId?.trim() && cfg.secretAccessKey?.trim() && cfg.bucket?.trim()
  );
}
var R2StorageClient;
var init_R2StorageClient = __esm({
  "connections/R2StorageClient.ts"() {
    R2StorageClient = class {
      ...
      async createMultipartUpload(params) {
        const command = new clientS3.CreateMultipartUploadCommand({
          ...
          ...params.fileHash ? { Metadata: { "x-amz-meta-file-hash": params.fileHash } } : {}
        });
      ...
      async signPart(params) {
        ...
        const url = await s3RequestPresigner.getSignedUrl(this.client, command, {
      ...
      async listParts(params) {
        const command = new clientS3.ListPartsCommand({
          Bucket: this.cfg.bucket,
          Key: params.key,
          UploadId: params.uploadId
        });
      ...
      async abortMultipartUpload(params) {
        const command = new clientS3.AbortMultipartUploadCommand({
          Bucket: this.cfg.bucket,
          Key: params.key,
          UploadId: params.uploadId
        });
...
var supabaseServiceClientSingleton, supabaseServiceClientConnection, cacheServiceConnection, cacheInvalidationServiceConnection;
var init_connections = __esm({
  "connections/index.ts"() {
    ...
    supabaseServiceClientConnection = new Proxy({}, {
      get(_target, prop, receiver) {
    ...
    cacheServiceConnection = cacheService;
    cacheInvalidationServiceConnection = cacheInvalidationService;
  }
});

// errors/AuthError.ts
var AuthError, AuthValidationError, InvalidCredentialsError, OAuthSignInRequiredError, TokenError, MissingUserIdError, NotVerifiedUserError, AuthNotFoundError, UserConflictError, IncorrectUserIDError, PermissionError;
var init_AuthError = __esm({
  "errors/AuthError.ts"() {
    ...
    AuthValidationError = class extends AuthError {
    ...
    InvalidCredentialsError = class extends AuthError {
    ...
    OAuthSignInRequiredError = class extends AuthError {
      constructor(message) {
        super(message, 401);
    ...
    TokenError = class extends AuthError {
      isTokenExpired;
    ...
    MissingUserIdError = class extends AuthError {
    ...
    NotVerifiedUserError = class extends AuthError {
    ...
    AuthNotFoundError = class extends AuthError {
    ...
    UserConflictError = class extends AuthError {
    ...
    IncorrectUserIDError = class extends AuthError {
    ...
    PermissionError = class extends AuthError {
...
var ValidationError, InfraError, DatabaseError, DatabaseEntityNotFoundError;
var init_InfraError = __esm({
  "errors/InfraError.ts"() {
    ...
    DatabaseEntityNotFoundError = class extends DatabaseError {
...
var init_AbstractEmailTemplate = __esm({
  "emails/AbstractEmailTemplate.ts"() {
    AbstractEmailTemplate = class {
    };
  }
});
...
var AuthUserDTOMapper;
var init_AuthUserDTO = __esm({
  "utils/dtos/AuthUserDTO.ts"() {
    AuthUserDTOMapper = class {
      /**
...
var init_AuthController = __esm({
  "controllers/AuthController.ts"() {
    init_AuthError();
    init_InfraError();
    init_VerifyEmailTemplate();
    init_WelcomeEmailTemplate();
    ...
    AuthController = class {
      ...
      googleOAuthCallback = async (req, res, next) => {
        try {
          ...
          const fullName = typeof meta?.full_name === "string" && meta.full_name.trim() || typeof meta?.name === "string" && meta.name.trim() || email;
          const googleIdentity = authUser.identities?.find((i) => i.provider === "google");
          const idData = googleIdentity?.identity_data;
          const providerId = typeof idData?.sub === "string" && idData.sub || (googleIdentity?.id != null ? String(googleIdentity.id) : authUser.id);
...
var init_UserError = __esm({
  "errors/UserError.ts"() {
    init_AppError();
    UserError = class extends AppError {
      constructor(message, statusCode, options2 = {}) {
        super(message, statusCode, { ...options2, errorCode: options2.errorCode ?? "USER_ERROR" });
        this.name = "UserError";
      }
    };
    UserNotFoundError = class extends UserError {
    ...
    UserAuthorizationError = class extends UserError {
    ...
    UserValidationError = class extends UserError {
...
var ChangePasswordEmailTemplate;
...
var VALID_PERMISSIONS, VALID_ROLES;
var init_rbacTypes = __esm({
  "data/types/rbacTypes.ts"() {
    VALID_PERMISSIONS = ["users.manage_roles"];
    VALID_ROLES = ["editor", "support", "admin"];
  }
});

// errors/RoleError.ts
var RoleValidationError;
var init_RoleError = __esm({
  "errors/RoleError.ts"() {
    init_AppError();
    RoleValidationError = class extends AppError {
      constructor(message, options2 = {}) {
...
var init_RbacService = __esm({
  "services/RbacService.ts"() {
    ...
    RbacService = class _RbacService {
...
var init_FeedbackController = __esm({
  "controllers/FeedbackController.ts"() {
    init_FeedbackDTO();
    init_InfraError();
...
var TABLE_NAME2, CORE_USER_SELECT, USER_WITH_PROFILE_SELECT, ADMIN_USER_SELECT, UserRepository;
var init_UserRepository = __esm({
  "repositories/UserRepository.ts"() {
    ...
    UserRepository = class {
      ...
      async getLastReadNotifications(userId) {
        const { data, error } = await this.supabase.from(TABLE_NAME2).select("last_read_notifications").eq("id", userId).maybeSingle();
        if (error) {
        ...
        return { lastReadNotifications: data.last_read_notifications };
      }
      /**
       * Advance last_read_notifications to now; returns the previous value for listing deltas.
       */
      async advanceLastReadNotifications(userId) {
        const prior = await this.getLastReadNotifications(userId);
        const now = (/* @__PURE__ */ new Date()).toISOString();
        const { error } = await this.supabase.from(TABLE_NAME2).update({ last_read_notifications: now, updated_at: now }).eq("id", userId);
        if (error) {
...
var init_ConfigRepository = __esm({
  "repositories/ConfigRepository.ts"() {
    init_InfraError();
    ConfigRepository = class _ConfigRepository {
  ...
  "repositories/OrganizationRepository.ts"() {
    ...
    OrganizationRepository = class {
      ...
      async findMembership(userId, organizationId) {
        const { data, error } = await this.supabase.from(USER_ORGS_TABLE).select(USER_ORG_SELECT).eq("user_id", userId).eq("organization_id", organizationId).maybeSingle();
        if (error) {
      ...
      async listMembersForNotificationEmails(organizationId) {
        const { data: uoList, error: uoError } = await this.supabase.from(USER_ORGS_TABLE).select("user_id").eq("organization_id", organizationId).eq("disabled", false);
        if (uoError) {
        ...
        const userIds = [...new Set((uoList ?? []).map((r) => r.user_id))];
        if (userIds.length === 0) return [];
        const { data: userList, error: userError } = await this.supabase.from("users").select("id, email, send_success_emails, send_failure_emails").in("id", userIds);
        if (userError) {
...
var RbacRepository;
var init_RbacRepository = __esm({
  "repositories/RbacRepository.ts"() {
    init_InfraError();
    RbacRepository = class _RbacRepository {
      constructor(supabase2) {
        this.supabase = supabase2;
      }
...
var TABLE, COLS, FeedbackRepository;
var init_FeedbackRepository = __esm({
  "repositories/FeedbackRepository.ts"() {
    init_InfraError();
    TABLE = "feedback";
    COLS = "id, feedback_type, url, description, email, is_handled, created_at";
...
var init_BlogRepository = __esm({
  "repositories/BlogRepository.ts"() {
    ...
    TABLE_NAME_BLOG_POSTS = "blog_posts";
    TABLE_NAME_BLOG_TOPICS = "blog_topics";
    TABLE_NAME_BLOG_COMMENTS = "blog_comments";
    TABLE_NAME_BLOG_ACTIVITIES = "blog_activities";
    ALLOWED_PUBLISHED_POST_SORT_KEYS = /* @__PURE__ */ new Set([
      "published_at",
      "created_at",
      "updated_at",
      "title",
    ...
    ALLOWED_ADMIN_POST_SORT_KEYS = /* @__PURE__ */ new Set([
      "created_at",
      "updated_at",
      "published_at",
      "title",
    ...
    ALLOWED_ADMIN_COMMENT_SORT_KEYS = /* @__PURE__ */ new Set(["created_at", "updated_at", "is_approved"]);
    ALLOWED_ADMIN_ACTIVITY_SORT_KEYS = /* @__PURE__ */ new Set(["created_at", "activity_type"]);
    SELECT_BLOG_TOPIC = `
  id,
  name,
  slug,
...
var init_StorageR2Repository = __esm({
  "repositories/StorageR2Repository.ts"() {
    init_InfraError();
    init_R2StorageClient();
    COMPOSER_MEDIA_BUCKET_NAME = "social_media";
    StorageR2Repository = class {
      client;
      constructor(r2Connection2) {
        this.client = r2Connection2 ? new R2StorageClient(r2Connection2) : null;
      }
      isConfigured() {
...
function isMissingVirtualFoldersTable(error) {
...
var TABLE_MEDIA, TABLE_MEDIA_VIRTUAL_FOLDERS, MediaRepository;
var init_MediaRepository = __esm({
  "repositories/MediaRepository.ts"() {
    ...
    init_InfraError();
    TABLE_MEDIA = "media";
    TABLE_MEDIA_VIRTUAL_FOLDERS = "media_virtual_folders";
    MediaRepository = class {
      constructor(supabase2) {
        this.supabase = supabase2;
      }
      async getMedia(organizationId, page, pageSize) {
        const safePageSize = Math.min(100, Math.max(1, Math.trunc(pageSize)));
        const safePage = Math.max(1, Math.trunc(page));
        const { count, error: countError } = await this.supabase.from(TABLE_MEDIA).select("id", { count: "exact", head: true }).eq("organization_id", organizationId).is("deleted_at", null);
        if (countError) {
        ...
        ).eq("organization_id", organizationId).is("deleted_at", null).order("created_at", { ascending: false }).range(boundedFrom, boundedTo);
        if (error) {
      ...
      async saveFile(params) {
        ...
        const { data, error } = await this.supabase.from(TABLE_MEDIA).insert(payload).select("id, path").single();
        if (error || !data) {
      ...
      async getMediaById(organizationId, id) {
        const { data, error } = await this.supabase.from(TABLE_MEDIA).select(
          "id, name, original_name, path, virtual_path, organization_id, created_at, updated_at, deleted_at, file_size, type, thumbnail, alt, thumbnail_timestamp"
      ...
      async softDeleteMedia(organizationId, id) {
        const now = (/* @__PURE__ */ new Date()).toISOString();
        const { data, error } = await this.supabase.from(TABLE_MEDIA).update({ deleted_at: now, updated_at: now }).eq("organization_id", organizationId).eq("id", id).is("deleted_at", null).select("id").maybeSingle();
        if (error) {
      ...
      async saveMediaInformation(organizationId, dto) {
        const updated_at = (/* @__PURE__ */ new Date()).toISOString();
        const payload = { updated_at };
        if (dto.alt !== void 0) payload.alt = dto.alt;
        if (dto.thumbnail !== void 0) payload.thumbnail = dto.thumbnail;
        if (dto.thumbnailTimestamp !== void 0) payload.thumbnail_timestamp = dto.thumbnailTimestamp;
        const { data, error } = await this.supabase.from(TABLE_MEDIA).update(payload).eq("organization_id", organizationId).eq("id", dto.id).select("id, path").single();
        if (error || !data) {
      ...
      async listAllMedia(organizationId) {
        const { data, error } = await this.supabase.from(TABLE_MEDIA).select(
          "id, name, original_name, path, virtual_path, organization_id, created_at, updated_at, deleted_at, file_size, type, thumbnail, alt, thumbnail_timestamp"
      ...
      async duplicateMedia(organizationId, sourceIds, targetVirtualPath) {
        ...
        for (const mediaId of sourceIds) {
          ...
          if (!source) continue;
          const { data, error } = await this.supabase.from(TABLE_MEDIA).insert({
      ...
      async updateVirtualPaths(organizationId, updates) {
        if (!updates.length) return 0;
        const now = (/* @__PURE__ */ new Date()).toISOString();
        let changed = 0;
        for (const row of updates) {
          const { data, error } = await this.supabase.from(TABLE_MEDIA).update({
            virtual_path: normalizeMediaVirtualPath(row.virtualPath),
            updated_at: now
          }).eq("organization_id", organizationId).eq("id", row.id).is("deleted_at", null).select("id").maybeSingle();
          if (error) {
      ...
      async listVirtualFolderPaths(organizationId) {
        const { data, error } = await this.supabase.from(TABLE_MEDIA_VIRTUAL_FOLDERS).select("path").eq("organization_id", organizationId).order("path", { ascending: true });
        if (error) {
        ...
        return (data ?? []).map((row) => normalizeMediaVirtualPath(String(row.path)));
      }
      async createVirtualFolder(organizationId, path7) {
        const normalized = normalizeMediaVirtualPath(path7);
        const { data, error } = await this.supabase.from(TABLE_MEDIA_VIRTUAL_FOLDERS).insert({ organization_id: organizationId, path: normalized }).select("id").maybeSingle();
        if (error) {
          if (error.code === "23505") {
            return true;
          }
      ...
      async deleteVirtualFolder(organizationId, path7) {
        const normalized = normalizeMediaVirtualPath(path7);
        if (isProtectedMediaVirtualFolder(normalized)) {
          return { deleted: false, message: "This folder cannot be deleted." };
        }
        const { data: mediaRows, error: mediaError } = await this.supabase.from(TABLE_MEDIA).select("virtual_path").eq("organization_id", organizationId).is("deleted_at", null);
        if (mediaError) {
        ...
        const hasMedia = (mediaRows ?? []).some((row) => {
          const vp = resolveMediaVirtualPath(String(row.virtual_path));
        ...
        const { data, error } = await this.supabase.from(TABLE_MEDIA_VIRTUAL_FOLDERS).delete().eq("organization_id", organizationId).eq("path", normalized).select("id").maybeSingle();
        if (error) {
          if (isMissingVirtualFoldersTable(error)) {
      ...
      async renameMediaDisplayName(organizationId, id, originalName) {
        const now = (/* @__PURE__ */ new Date()).toISOString();
        const { data, error } = await this.supabase.from(TABLE_MEDIA).update({ original_name: originalName, updated_at: now }).eq("organization_id", organizationId).eq("id", id).is("deleted_at", null).select("id").maybeSingle();
        if (error) {
...
var init_IntegrationRepository = __esm({
  "repositories/IntegrationRepository.ts"() {
    ...
    IntegrationRepository = class {
      ...
      async customers(organizationId) {
        const { data, error } = await this.supabase.from("integration_customers").select("id, name").eq("organization_id", organizationId).order("name", { ascending: true });
        if (error) {
      ...
      async createIntegrationCustomer(organizationId, name) {
        const trimmed = name.trim();
        const { data, error } = await this.supabase.from("integration_customers").insert({
          organization_id: organizationId,
          name: trimmed,
          updated_at: (/* @__PURE__ */ new Date()).toISOString()
       ...
       */
      async updateOnCustomerName(organizationId, integrationId, name) {
        const trimmed = name.trim();
        if (!trimmed) {
          await this.updateIntegrationGroup(organizationId, integrationId, null);
          return;
        }
        const { data: existing, error: findError } = await this.supabase.from("integration_customers").select("id").eq("organization_id", organizationId).eq("name", trimmed).maybeSingle();
        if (findError) {
      ...
      async updateIntegrationById(organizationId, integrationId, params) {
        ...
        const { data, error } = await this.supabase.from(TABLE2).update({
          ...
          refresh_needed: false,
          updated_at: (/* @__PURE__ */ new Date()).toISOString()
        }).eq("organization_id", organizationId).eq("id", integrationId).is("deleted_at", null).select("*").single();
        if (error || !data) {
...
var PLUGS_TABLE, PlugRepository;
var init_PlugRepository = __esm({
  "repositories/PlugRepository.ts"() {
    init_InfraError();
    PLUGS_TABLE = "plugs";
    PlugRepository = class {
      constructor(supabase2) {
        this.supabase = supabase2;
      }
    ...
    PostsRepository = class {
      ...
      async insertTag(organizationId, nameTrimmed, color) {
        ...
        const { data, error } = await this.supabase.from(TABLE_TAGS).insert({
          ...
          created_at: now,
          updated_at: now
        }).select("id, name, color, org_id, deleted_at, created_at, updated_at").single();
        if (error || !data) {
      ...
      async listTagNamesByPostIds(postIds) {
        const out = /* @__PURE__ */ new Map();
        if (postIds.length === 0) return out;
        const { data: links, error: linksErr } = await this.supabase.from(TABLE_POSTS_TAGS).select("post_id, tag_id").in("post_id", postIds);
        if (linksErr) {
          throw new DatabaseError(`Failed to load tag assignments: ${linksErr.message}`, {
            cause: linksErr,
            operation: "select",
...
var TABLE_APPS, TABLE_AUTHS, OauthAppRepository;
var init_OauthAppRepository = __esm({
  "repositories/OauthAppRepository.ts"() {
    init_InfraError();
    TABLE_APPS = "oauth_apps";
    TABLE_AUTHS = "oauth_authorizations";
    OauthAppRepository = class {
      constructor(supabase2) {
        this.supabase = supabase2;
      }
...
var init_SubscriptionRepository = __esm({
  "repositories/SubscriptionRepository.ts"() {
    init_InfraError();
    SUBSCRIPTIONS_TABLE = "organization_subscriptions";
    ORGS_TABLE2 = "organizations";
    SubscriptionRepository = class {
      ...
      async getOrganizationBilling(organizationId) {
        const { data, error } = await this.supabase.from(ORGS_TABLE2).select("id, name, stripe_customer_id, allow_trial, is_trialing").eq("id", organizationId).maybeSingle();
        if (error) {
      ...
      async getOrganizationByStripeCustomerId(customerId) {
        const { data, error } = await this.supabase.from(ORGS_TABLE2).select("id, name, stripe_customer_id, allow_trial, is_trialing").eq("stripe_customer_id", customerId).maybeSingle();
        if (error) {
      ...
      async updateStripeCustomerId(organizationId, customerId) {
        const { error } = await this.supabase.from(ORGS_TABLE2).update({ stripe_customer_id: customerId, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", organizationId);
        if (error) {
      ...
      async softDeleteByOrganizationId(organizationId) {
        const now = (/* @__PURE__ */ new Date()).toISOString();
        const { error } = await this.supabase.from(SUBSCRIPTIONS_TABLE).update({ deleted_at: now, updated_at: now }).eq("organization_id", organizationId).is("deleted_at", null);
        if (error) {
      ...
      async softDeleteByStripeCustomerId(customerId) {
...
var init_repositories = __esm({
  "repositories/index.ts"() {
    ...
    init_IntegrationRepository();
    init_PlugRepository();
    init_NotificationRepository();
    init_PostsRepository();
    init_SignatureRepository();
    init_SetsRepository();
    ...
    organizationRepository = new OrganizationRepository(supabaseServiceClientConnection);
    ...
    storageR2Repository = new StorageR2Repository(r2Connection);
    mediaRepository = new MediaRepository(supabaseServiceClientConnection);
    storageSupabaseRepository = new StorageSupabaseRepository(supabaseServiceClientConnection);
    integrationRepository = new IntegrationRepository(supabaseServiceClientConnection);
    plugRepository = new PlugRepository(supabaseServiceClientConnection);
    notificationRepository = new NotificationRepository(supabaseServiceClientConnection);
    postsRepository = new PostsRepository(supabaseServiceClientConnection);
    signatureRepository = new SignatureRepository(supabaseServiceClientConnection);
...
var init_UserService = __esm({
  "services/UserService.ts"() {
    ...
    BLOG_PUBLISHED_AUTHORS_KEY = "blog:published:authors";
    ...
    UserService = class {
       ...
       */
      async invalidateCachesAfterEmailVerification(authUserId, userEmail) {
        await this._invalidateUserRelatedCaches({
          authUserId,
...
var CACHE_KEYS3, COMPANY_CACHE_TTL, CompanyService;
var init_CompanyService = __esm({
  "services/CompanyService.ts"() {
    CACHE_KEYS3 = {
      CONFIG_COMPANY_INFORMATION: "config:module:company_information"
    };
    COMPANY_CACHE_TTL = 300;
    CompanyService = class _CompanyService {
...
var CACHE_KEYS4, MARKETING_CACHE_TTL_SEC, MarketingService;
var init_MarketingService = __esm({
  "services/MarketingService.ts"() {
    CACHE_KEYS4 = {
      CONFIG_MARKETING_INFORMATION: "config:module:marketing_information"
    };
    MARKETING_CACHE_TTL_SEC = 300;
    MarketingService = class _MarketingService {
      constructor(configRepository2, cache) {
        this.configRepository = configRepository2;
        this.cache = cache;
      }
...
var OrganizationInviteEmailTemplate;
...
var CACHE_KEYS6, FEEDBACK_CACHE_TTL_SEC, FeedbackService;
...
var CACHE_KEYS7, BLOG_CACHE_TTL_SEC, BlogService;
...
var init_ProviderIntegrationErrors = __esm({
  "errors/ProviderIntegrationErrors.ts"() {
    ProviderAccessTokenExpiredError = class extends Error {
...
function throwIfMetaGraphInvalidAccessToken(json) {
  const e = json.error;
  if (!e) return;
  const code = e.code;
...
var init_instagramStandaloneProvider = __esm({
  "integrations/providers/instagramStandaloneProvider.ts"() {
    ...
    IG_INSIGHTS_GRAPH2 = "https://graph.instagram.com/v21.0";
    InstagramStandaloneProvider = class {
      ...
      async generateAuthUrl() {
        ...
        const url = `https://www.instagram.com/oauth/authorize?client_id=${encodeURIComponent(appId)}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=${encodeURIComponent(this.scopes.join(","))}&state=${encodeURIComponent(state)}`;
        return { url, codeVerifier, state };
      }
      async authenticate(params) {
        ...
        const longRes = await fetch(
          `${IG_GRAPH}/access_token?grant_type=ig_exchange_token&client_id=${encodeURIComponent(appId)}&client_secret=${encodeURIComponent(appSecret)}&access_token=${encodeURIComponent(shortToken)}`
        );
  ...
  "integrations/providers/threadsProvider.ts"() {
    ...
    ThreadsProvider = class {
      ...
      async generateAuthUrl() {
        ...
        const url = `https://www.threads.net/oauth/authorize?client_id=${appId}&redirect_uri=${encodeURIComponent(redirectUri)}&state=${state}&scope=${encodeURIComponent(this.scopes.join(","))}`;
        return { url, codeVerifier, state };
      }
      async authenticate(params) {
      ...
      async checkLoaded(creationId, accessToken) {
        for (let i = 0; i < 40; i++) {
          ...
          if (status === "ERROR") {
            const detail = json.error_message || "Threads media processing failed";
            const raw = JSON.stringify(json).slice(0, 2e3);
            throw new Error(
...
var init_mirrorIntegrationProfilePicture = __esm({
  "utils/mirrorIntegrationProfilePicture.ts"() {
    ...
    init_Logger();
    INTEGRATION_PROFILE_STORAGE_PREFIX = "integration-profiles";
  }
});
...
var init_RefreshIntegrationService = __esm({
  "services/RefreshIntegrationService.ts"() {
    ...
    RefreshIntegrationService = class {
      ...
      async informAboutRefreshError(integration) {
...
var init_IntegrationService = __esm({
  "services/IntegrationService.ts"() {
    ...
    ANALYTICS_CACHE_TTL_SEC = !process.env.NODE_ENV || process.env.NODE_ENV === "development" ? 1 : 3600;
    INTEGRATION_CUSTOMERS_LIST_TTL_SEC = !process.env.NODE_ENV || process.env.NODE_ENV === "development" ? 5 : 300;
    IntegrationService = class {
      ...
      async invalidateIntegrationCustomersListCache(organizationId) {
...
var init_PlugService = __esm({
  "services/PlugService.ts"() {
    ...
    PLUG_CACHE_TTL_SEC = !process.env.NODE_ENV || process.env.NODE_ENV === "development" ? 15 : 90;
    PlugService = class {
...
var init_IntegrationConnectionService = __esm({
  "services/IntegrationConnectionService.ts"() {
    ...
    IntegrationConnectionService = class {
      ...
      async createIntegrationCustomer(authUserId, organizationId, name) {
        await this.assertOrganizationMember(authUserId, organizationId);
        return this.integrations.createIntegrationCustomer(organizationId, name);
      }
      async assignIntegrationCustomer(authUserId, organizationId, integrationId, customerId) {
        await this.assertOrganizationMember(authUserId, organizationId);
...
var init_NotificationService = __esm({
  "services/NotificationService.ts"() {
    init_GlobalConfig();
    init_notificationTransactionalEmailHtml();
    init_OrganizationError();
    init_UserError();
    init_Logger();
    NotificationService = class {
      ...
      async requireActiveMember(authUserId, organizationId) {
        const { userId } = await this.organizationRepository.findUserIdByAuthId(authUserId);
        if (!userId) {
          throw new UserNotFoundError(authUserId);
        }
      ...
      async getMainPageCount(authUserId, organizationId) {
      ...
      async getNotificationList(authUserId, organizationId) {
        const userId = await this.requireActiveMember(authUserId, organizationId);
      ...
      async inAppNotification(organizationId, subject, message, sendEmail = false, digest = false, type = "success") {
        ...
        try {
          await this.transactionalNotificationEmail.deliverToOrganizationMembers({
            ...
            htmlBodyInner: inner,
            type,
            deliver: transport === "bullmq" ? async (to, subj, html) => {
...
// services/TransactionalNotificationEmailService.ts
function shouldSendEmailForMember(type, sendSuccessEmails, sendFailureEmails) {
  if (type === "info") return true;
  if (type === "success") return sendSuccessEmails;
  if (type === "fail") return sendFailureEmails;
  return true;
}
var TransactionalNotificationEmailService;
var init_TransactionalNotificationEmailService = __esm({
  "services/TransactionalNotificationEmailService.ts"() {
    init_createQueueIoredis();
    init_notificationTransactionalEmailHtml();
    init_Logger();
    TransactionalNotificationEmailService = class {
      ...
      async enqueueSendPlainJob(payload) {
...
var init_PostsService = __esm({
  "services/PostsService.ts"() {
    ...
    PostsService = class {
      ...
      async deleteTag(organizationId, authUserId, tagId) {
        await this.integrationConnectionService.assertOrganizationMember(authUserId, organizationId);
...
var init_MediaService = __esm({
  "services/MediaService.ts"() {
    MediaService = class {
      ...
      getMedia(organizationId, page, pageSize) {
      ...
      listAllMedia(organizationId) {
        return this._mediaRepository.listAllMedia(organizationId);
      }
      updateVirtualPaths(organizationId, updates) {
        return this._mediaRepository.updateVirtualPaths(organizationId, updates);
      }
      duplicateMedia(organizationId, sourceIds, targetVirtualPath) {
        return this._mediaRepository.duplicateMedia(organizationId, sourceIds, targetVirtualPath);
      }
      renameMediaDisplayName(organizationId, id, originalName) {
        return this._mediaRepository.renameMediaDisplayName(organizationId, id, originalName);
      }
      listVirtualFolderPaths(organizationId) {
        return this._mediaRepository.listVirtualFolderPaths(organizationId);
      }
      createVirtualFolder(organizationId, path7) {
        return this._mediaRepository.createVirtualFolder(organizationId, path7);
      }
      deleteVirtualFolder(organizationId, path7) {
        return this._mediaRepository.deleteVirtualFolder(organizationId, path7);
      }
    };
  }
});
...
var SignatureNotFoundError;
var init_SignatureError = __esm({
  "errors/SignatureError.ts"() {
    init_AppError();
    SignatureNotFoundError = class extends AppError {
...
// errors/SetError.ts
var SetNotFoundError;
var init_SetError = __esm({
  "errors/SetError.ts"() {
    init_AppError();
    SetNotFoundError = class extends AppError {
      constructor(setId) {
...
var OauthAppService;
var init_OauthAppService = __esm({
  "services/OauthAppService.ts"() {
    init_MediaRepository();
    init_AppError();
    init_make_is();
...
var init_SubscriptionError = __esm({
  "errors/SubscriptionError.ts"() {
    SubscriptionError = class extends Error {
      ...
      section;
      billingUrl;
      constructor(message, section, billingUrl) {
        super(message);
...
var init_StripeService = __esm({
  "services/StripeService.ts"() {
    ...
    StripeService = class {
      ...
      async assertPriceMatchesPlan(priceId, tier, period) {
        const stripe = getStripeClient();
        const price = await stripe.prices.retrieve(priceId);
        const limits = pricing[tier];
        const expectedCents = (period === "MONTHLY" ? limits.month_price : limits.year_price) * 100;
      ...
      async findOrCreatePrice(tier, period) {
        const stripe = getStripeClient();
        const limits = pricing[tier];
        const amountUsd = period === "MONTHLY" ? limits.month_price : limits.year_price;
        const interval = period === "MONTHLY" ? "month" : "year";
        const nickname = `${tier} ${period}`;
        const products = await stripe.products.list({ active: true, limit: 100 });
        let product = products.data.find((p) => p.name?.toUpperCase() === tier) ?? await stripe.products.create({ name: tier, active: true });
        const prices = await stripe.prices.list({ active: true, product: product.id, limit: 100 });
        const existing = prices.data.find(
          (p) => p.recurring?.interval === interval && p.nickname === nickname && p.unit_amount === amountUsd * 100
        );
...
var init_services = __esm({
  "services/index.ts"() {
    ...
    init_PlugService();
    init_IntegrationConnectionService();
    init_NotificationService();
    init_TransactionalNotificationEmailService();
    init_PostsService();
    init_MediaService();
    init_SignatureService();
    init_SetsService();
    ...
    refreshIntegrationService = new RefreshIntegrationService(
...
function ensureFolderPath(segments, folders) {
  let acc = "";
...
function buildMediaTreeEntities(items, extraFolderPaths = []) {
  const folderPaths = new Set(DEFAULT_FOLDERS);
  ...
  for (const item of items) {
    ...
    const date = item.lastModified ? new Date(item.lastModified) : /* @__PURE__ */ new Date();
    entities.push({
      id,
      type: "file",
      size: item.size,
...
function readVirtualPath(body) {
  const raw = typeof body?.virtualPath === "string" ? String(body.virtualPath) : typeof body?.virtual_path === "string" ? String(body.virtual_path) : "";
  return raw.trim() ? normalizeMediaVirtualPath(raw) : void 0;
}
...
var init_MediaController = __esm({
  "controllers/MediaController.ts"() {
    ...
    MediaController = class {
      constructor(mediaService2, subscriptionService2, storageR2Repository2, uploadProvider) {
      ...
      list = async (req, res, next) => {
        try {
          ...
          const pageSize = Number.isFinite(rawPageSize) ? Math.min(100, Math.max(1, Math.trunc(rawPageSize))) : 24;
      ...
      deleteFolder = async (req, res, next) => {
        try {
          ...
          const path7 = normalizeMediaVirtualPath(pathRaw);
          const result = await this.mediaService.deleteVirtualFolder(organizationId, path7);
          if (!result.deleted) {
            throw new UserValidationError(result.message);
          }
          res.status(200).json({ success: true, data: { path: path7 } });
        } catch (error) {
          next(error);
        }
      };
      createFolder = async (req, res, next) => {
        try {
          ...
          const path7 = normalizeMediaVirtualPath(`${parent}/${nameRaw}`);
          const created = await this.mediaService.createVirtualFolder(organizationId, path7);
          if (!created) {
            throw new UserValidationError(
      ...
      multipart = async (req, res, next) => {
        try {
          ...
          if (endpoint === "create-multipart-upload") {
            ...
            const fileHash = typeof req.body?.fileHash === "string" ? String(req.body.fileHash) : void 0;
          ...
          if (endpoint === "prepare-upload-parts") {
            ...
            const out = await this.storageR2Repository.prepareUploadParts({
              key,
              uploadId,
              parts: parts.map((p) => ({ number: Number(p?.number) }))
            });
            res.status(200).json(out);
            return;
          }
          if (endpoint === "sign-part") {
            ...
            const partNumber = Number.parseInt(String(req.body?.partNumber ?? "0"), 10);
      ...
      uploadSimple = async (req, res, next) => {
        try {
          ...
          const preventSave = typeof req.body?.preventSave === "string" ? String(req.body.preventSave).toLowerCase() === "true" : Boolean(req.body?.preventSave);
          const file = req.file;
          const organizationId = typeof req.body?.organizationId === "string" ? String(req.body.organizationId) : "";
          if (!organizationId.trim()) {
            throw new UserValidationError("organizationId is required");
      ...
      delete = async (req, res, next) => {
        try {
          ...
          const { id, path: objectPath } = req.body ?? {};
          const mediaRow = id ? await this.mediaService.getMediaById(organizationId, id) : objectPath ? await this.mediaService.getMediaByPath(organizationId, objectPath) : null;
          if (!mediaRow) {
      ...
      saveMediaInformation = async (req, res, next) => {
        try {
          const authUser = req.user;
          if (!authUser?.id) throw new UserValidationError("Authentication required");
          const organizationId = typeof req.body?.organizationId === "string" ? String(req.body.organizationId) : "";
          ...
          const dto = {
            id,
            alt: req.body?.alt ?? void 0,
            thumbnail: req.body?.thumbnail ?? void 0,
            thumbnailTimestamp: req.body?.thumbnailTimestamp ?? void 0
          };
      ...
      saveMedia = async (req, res, next) => {
        try {
          ...
          const originalName = typeof req.body?.originalName === "string" ? req.body.originalName : void 0;
          if (!name) {
            throw new UserValidationError("name is required");
          }
          const saved = await this.mediaService.saveFile({
            ...
            type: typeof req.body?.type === "string" ? String(req.body.type) : void 0
          });
          res.status(200).json({
            success: true,
            data: {
      ...
      handle = async (req, res, next) => {
        try {
          ...
          if (!Buffer.isBuffer(rawBody) || rawBody.length === 0) {
            res.status(400).json({
              success: false,
              message: "Stripe webhook body was not captured as raw bytes. Use POST directly on the API (e.g. stripe listen --forward-to localhost:3000/api/v1/billing/webhooks/stripe)."
            });
            return;
          }
...
var init_ConfigController = __esm({
  "controllers/ConfigController.ts"() {
    ConfigController = class _ConfigController {
...
var EmailController;
var init_EmailController = __esm({
  "controllers/EmailController.ts"() {
...
var init_IntegrationController = __esm({
  "controllers/IntegrationController.ts"() {
    ...
    IntegrationController = class {
      ...
      getChannelCustomers = async (req, res, next) => {
        try {
          const authReq = req;
          const authUserId = authReq.user?.id;
          if (!authUserId) {
      ...
      createChannelCustomer = async (req, res, next) => {
        try {
          const authReq = req;
          const authUserId = authReq.user?.id;
          if (!authUserId) {
      ...
      assignChannelCustomer = async (req, res, next) => {
        try {
          const authReq = req;
          const authUserId = authReq.user?.id;
          if (!authUserId) {
      ...
      saveProviderPage = async (req, res, next) => {
        try {
          const authReq = req;
          const authUserId = authReq.user?.id;
          if (!authUserId) {
...
var PublicIntegrationController;
var init_PublicIntegrationController = __esm({
  "controllers/PublicIntegrationController.ts"() {
    init_connections();
    PublicIntegrationController = class {
...
var init_NotificationController = __esm({
  "controllers/NotificationController.ts"() {
    ...
    NotificationController = class {
       ...
       */
      mainPageCount = async (req, res, next) => {
        try {
          const authReq = req;
          const authUserId = authReq.user?.id;
          if (!authUserId) {
...
var PublicNotificationController;
var init_PublicNotificationController = __esm({
  "controllers/PublicNotificationController.ts"() {
    init_connections();
    PublicNotificationController = class {
      constructor(notificationService2) {
        this.notificationService = notificationService2;
      }
...
var PostsController;
var init_PostsController = __esm({
  "controllers/PostsController.ts"() {
    init_PostDTO();
    init_UserError();
    PostsController = class {
      ...
      createTag = async (req, res, next) => {
        try {
          ...
          const { organizationId, name, color } = req.body;
...
var PublicPostsController;
var init_PublicPostsController = __esm({
  "controllers/PublicPostsController.ts"() {
    init_PostDTO();
    init_connections();
    PublicPostsController = class {
      constructor(postsService2) {
        this.postsService = postsService2;
      }
...
var PublicAnalyticsController;
var init_PublicAnalyticsController = __esm({
  "controllers/PublicAnalyticsController.ts"() {
    init_connections();
    PublicAnalyticsController = class {
...
var OauthAppController;
var init_OauthAppController = __esm({
  "controllers/OauthAppController.ts"() {
    init_UserError();
    OauthAppController = class {
...
var OauthController;
var init_OauthController = __esm({
  "controllers/OauthController.ts"() {
    init_UserError();
...
var ApprovedAppsController;
var init_ApprovedAppsController = __esm({
  "controllers/ApprovedAppsController.ts"() {
    init_UserError();
    ApprovedAppsController = class {
      constructor(oauthService2) {
        this.oauthService = oauthService2;
      }
...
var init_SignatureController = __esm({
  "controllers/SignatureController.ts"() {
    init_UserError();
    init_SignatureDTO();
...
__export(controllers_exports, {
  ...
  publicAnalyticsController: () => publicAnalyticsController,
  publicIntegrationController: () => publicIntegrationController,
  publicNotificationController: () => publicNotificationController,
  publicPostsController: () => publicPostsController,
...
var init_controllers = __esm({
  "controllers/index.ts"() {
    ...
    init_StripeWebhookController();
    init_ConfigController();
    init_EmailController();
    init_IntegrationController();
    init_PublicIntegrationController();
    init_NotificationController();
    init_PublicNotificationController();
    init_PostsController();
    init_PublicPostsController();
    init_PublicAnalyticsController();
    init_OauthAppController();
    init_OauthController();
    init_ApprovedAppsController();
    init_ThirdPartyController();
    init_SignatureController();
    init_SetsController();
    ...
    integrationController = new IntegrationController(integrationConnectionService, integrationManager);
    publicIntegrationController = new PublicIntegrationController(integrationConnectionService);
    notificationController = new NotificationController(notificationService);
    publicNotificationController = new PublicNotificationController(notificationService);
    postsController = new PostsController(postsService);
    publicPostsController = new PublicPostsController(postsService);
    publicAnalyticsController = new PublicAnalyticsController(analyticsService, postsService);
    oauthAppController = new OauthAppController(oauthAppService);
    oauthController = new OauthController(oauthService);
    approvedAppsController = new ApprovedAppsController(oauthService);
...
function createNotificationOrganizationQueryParser() {
  return createQueryParser({
    organizationId: QueryParsers.string
  });
}
var notificationPageParser = (value) => {
...
function readFolderStructure(dirPath, previousFolder = "") {
  ...
  try {
    const dirents = fs2__default.default.readdirSync(dirPath, { withFileTypes: true });
...
init_connections();
init_repositories();
var mediaOrganizationQuerySchema = zod.z.object({
...
var saveMediaInformationBodySchema = zod.z.object({
  ...
  alt: zod.z.string().max(2e3).optional().nullable(),
  /** Public URL or storage object key written to `media.thumbnail`. */
  thumbnail: zod.z.string().max(2048).optional().nullable(),
  thumbnailTimestamp: zod.z.number().int().nonnegative().optional().nullable()
});
var validateSaveMediaInformationBody = validateRequest({
  body: saveMediaInformationBodySchema
});
var multipartEndpointParamsSchema = zod.z.object({
  endpoint: zod.z.enum([
...
var multipartBodySchema = zod.z.object({
  organizationId: zod.z.string().uuid("Invalid organization id")
}).passthrough();
var validateMultipartEndpoint = validateRequest({
  params: multipartEndpointParamsSchema,
  body: multipartBodySchema
});
var mediaMoveBodySchema = zod.z.object({
  organizationId: zod.z.string().uuid("Invalid organization id"),
  ids: zod.z.array(zod.z.string().min(1)).min(1),
  target: zod.z.string().min(1).max(512)
});
var validateMediaMoveBody = validateRequest({
  body: mediaMoveBodySchema
});
var mediaCopyBodySchema = mediaMoveBodySchema;
var validateMediaCopyBody = validateRequest({
  body: mediaCopyBodySchema
});
var mediaRenameBodySchema = zod.z.object({
  organizationId: zod.z.string().uuid("Invalid organization id"),
  id: zod.z.string().min(1).max(512),
  name: zod.z.string().trim().min(1).max(256)
});
var validateMediaRenameBody = validateRequest({
  body: mediaRenameBodySchema
});
var mediaCreateFolderBodySchema = zod.z.object({
  organizationId: zod.z.string().uuid("Invalid organization id"),
  parent: zod.z.string().min(1).max(512).optional(),
  name: zod.z.string().trim().min(1).max(128)
});
var validateMediaCreateFolderBody = validateRequest({
  body: mediaCreateFolderBodySchema
});
var mediaDeleteFolderBodySchema = zod.z.object({
  organizationId: zod.z.string().uuid("Invalid organization id"),
  path: zod.z.string().min(1).max(512)
});
var validateMediaDeleteFolderBody = validateRequest({
...
var socialConnectBodySchema = zod.z.object({
  ...
  refresh: zod.z.string().min(1).optional()
});
...
var saveProviderPageParamsSchema = zod.z.object({
  id: zod.z.string().uuid("Invalid integration id")
});
var saveProviderPageBodySchema = zod.z.object({
  organizationId: zod.z.string().uuid("Invalid organization id")
}).passthrough();
var validateSaveProviderPage = validateRequest({
...
var validateIntegrationCustomersQuery = validateRequest({
  query: integrationCustomersQuerySchema
});
var integrationCreateCustomerBodySchema = zod.z.object({
  organizationId: zod.z.string().uuid("Invalid organization id"),
  name: zod.z.string().trim().min(1, "Name is required").max(200)
});
var validateIntegrationCreateCustomerBody = validateRequest({
  body: integrationCreateCustomerBodySchema
});
var integrationGroupParamsSchema = zod.z.object({
  id: zod.z.string().uuid("Invalid integration id")
});
var integrationGroupBodySchema = zod.z.object({
  organizationId: zod.z.string().uuid("Invalid organization id"),
  /** Set to `null` to clear the customer assignment. */
  customerId: zod.z.union([zod.z.string().uuid("Invalid customer id"), zod.z.null()])
});
var validateIntegrationGroup = validateRequest({
...
var validateNotificationOrganizationQuery = validateRequest({
...
function validateBillingOrganizationQuery(req, res, next) {
...
function errorHandler(err, _req, res, _next) {
  ...
  let eventId;
  try {
    eventId = Sentry__namespace.captureException(err) ?? void 0;
    if (eventId) {
      logger.info({
...
function shouldParseStripeWebhookRaw(req) {
...
function stripeWebhookRawBodyMiddleware(bodyLimit) {
  return [
    express__default.default.raw({
    ...
    (req, res, next) => {
      ...
      if (!Buffer.isBuffer(body) || body.length === 0) {
        res.status(400).json({
          success: false,
          message: "Stripe webhook requires a raw JSON body. Forward to the API port directly (e.g. localhost:3000), not through a proxy that re-encodes JSON."
        });
        return;
      }
      req.rawBody = body;
...
async function createApp() {
  ...
  const apiPrefix = String(config2.api.prefix ?? "/api/v1").replace(/\/+$/, "");
  const bodyLimit = String(config2.server?.bodyLimit ?? "10mb");
  app.post(
    `${apiPrefix}/billing/webhooks/stripe`,
    ...stripeWebhookRawBodyMiddleware(bodyLimit),
    async (req, res, next) => {
      const { stripeWebhookController: stripeWebhookController2 } = await Promise.resolve().then(() => (init_controllers(), controllers_exports));
...
function waitForResponseEnd(res, run) {
