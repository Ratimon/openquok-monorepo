<script lang="ts">
	...
	import {
		...
		getRootPathSecretAdminListingManagerStackEditor
	...
	const getEditHref = (listing: ListingTableItem) =>
		url(getRootPathSecretAdminListingManagerStackEditor(listing.id));
...
<div class="p-4 md:p-6">
	<div class="flex items-start justify-between gap-4 flex-wrap">
		<div class="min-w-0">
			<h1 class="text-xl font-semibold text-base-content">Stacks</h1>
			<p class="text-sm text-base-content/70">Manage stack listings. Platform admin only.</p>
		</div>
	...
	{:else if !hasListings}
		...
		>
			<div class="flex flex-col items-center gap-1 text-center">
				<h3 class="text-2xl font-bold tracking-tight text-base-content">You have no stacks</h3>
				<p class="text-sm text-base-content/70">Create your first stack listing to get started.</p>
				<Button variant="primary" size="sm" href={newStackHref} class="mt-4">Add stack</Button>
			</div>
		</div>
	{:else}
		<ListingStacksTable
