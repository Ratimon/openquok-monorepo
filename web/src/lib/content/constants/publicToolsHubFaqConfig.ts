...
export const PUBLIC_TOOLS_HUB_FAQ = {
	...
	faqItems: appendPublicGeneralFaqItems(
		[
		{
			...
			description:
				`OpenQuok ships browser tools for agent skills, visuals, copy, and timing tests. Use ${faqLink(publicFaqHref.skillBuilderTool, 'Skill Builder')} to export SKILL.md and save a ${faqLink(publicFaqHref.playbooks, 'playbook')} draft — see ${faqLink(publicFaqHref.docsPlaybooksCompose, 'Compose a playbook')}. ${faqLink(publicFaqHref.photoEditorTool, 'Photo Editor')} sizes images, ${faqLink(publicFaqHref.humanizerTool, 'Humanizer')} rewrites copy with no sign up, and ${faqLink(publicFaqHref.bestTimeToPostTool, 'Best Time to Post')} plans timing experiments. Integrations API docs live under ${faqLink(publicFaqHref.publicApi, 'Public API')}.`
		},
		{
