...
export const PUBLIC_SELF_HOSTING_LANDING_CONFIG = {
		...
		cards: [
			...
			},
			{
				...
				bullets: [
					...
					'Build from the clone or pull API and workers from GHCR or Docker Hub',
		...
		items: [
			{
				...
				description: 'Run OpenQuok on one server — env file, Supabase, start, stop, and updates.',
				href: publicFaqHref.dockerCompose,
				ctaLabel: 'Open guide',
				iconName: icons.BookOpenCheck.name
			},
			{
				id: 'docker-pre-built-images',
				title: 'Docker pre-built images',
				description: 'Pull published API and worker images when you want a faster install.',
				href: publicFaqHref.dockerPreBuiltImages,
				ctaLabel: 'Open guide',
				iconName: icons.FolderInput.name
			},
			{
	...
	faqSection: {
		...
		faqDescription:
			'Common questions about cost, what you run, pre-built images, hosted vs self-host, social apps, agent login, and public servers.',
		faqItems: appendPublicGeneralFaqItems(
			[
				{
					title: 'What is the difference between Docker Compose and production deployment?',
					description:
						`Docker Compose path runs the whole app on one local server you control. Production deployment path spreads the same parts across cloud services you choose — for example the website on Vercel and background jobs on Railway. You use your own Supabase project in both cases. Neither path is the hosted plan on openquok.com. Start with the ${faqLink(publicFaqHref.dockerCompose, 'Docker Compose guide')} or ${faqLink(publicFaqHref.productionDeployment, 'production deployment guide')}.`
				},
				{
					title: 'Can I pull container images instead of building from source?',
					description:
						`Yes. We publish ready-made images for the API and background workers. You can download them instead of building those parts yourself. You usually still build the website when your Supabase keys or public URL differ from our defaults. See ${faqLink(publicFaqHref.dockerPreBuiltImages, 'Docker pre-built images')} and the ${faqLink(publicFaqHref.dockerCompose, 'Docker Compose guide')}.`
				},
				{
					title: 'Does self-hosting cost money?',
					description:
						`OpenQuok charges no software fee when you run it yourself. You still pay for servers, Supabase, bandwidth, and any social apps you register. ${faqLink(publicFaqHref.cloud, 'OpenQuok Cloud')} uses paid plans on ${faqLink(publicFaqHref.pricing, 'Pricing')} — see the ${faqLink(publicFaqHref.billing, 'billing guide')}. Compare paths on ${faqLink(publicSelfHostingPath, 'Self-host OpenQuok')}.`
				},
				{
					title: 'What services do I need to run?',
					description:
						`Docker Compose runs the website, API, queue, and workers on one machine. You can build from the repo or use ${faqLink(publicFaqHref.dockerPreBuiltImages, 'pre-built images')} for the API and workers. A split cloud stack uses the same roles on separate services — see ${faqLink(publicFaqHref.productionDeployment, 'production deployment')} and ${faqLink(configurationWorkerHref, 'worker configuration')}. You bring Supabase in both cases. Optional CLI setup lets agents sign in without API keys on the server. Start with ${faqLink(systemRequirementsHref, 'system requirements')}.`
				},
				{
					title: 'Is self-hosted OpenQuok the same product as hosted?',
					description:
						`Yes. You get the same calendar, kanban, composer, agents, Public API, and MCP tools. Plan limits on openquok.com apply only to the hosted cloud product. When you self-host, you skip hosted billing and use your own social developer apps. Compare paths on ${faqLink(publicSelfHostingPath, 'Self-host OpenQuok')}.`
				},
				{
				...
				},
				{
					...
					description:
						`Yes. Turn on the optional CLI profile in Docker Compose. Register one OAuth app for device login, then run openquok auth:login from the agent machine. See ${faqLink(configurationAgentHref, 'agent configuration')}, ${faqLink(oauthServerHref, 'OAuth server admin')}, or the ${faqLink(publicFaqHref.blogSelfHost, 'device-login walkthrough')}.`
				},
				{
					title: 'Is it safe to expose Docker Compose on a public VPS?',
					description:
						`The default setup suits your own computer or a private network. Before you open it on the public internet, use HTTPS, limit which ports are reachable, and follow the security notes in the ${faqLink(publicFaqHref.dockerCompose, 'Docker Compose guide')}.`
				}
			],
			PUBLIC_SELF_HOSTING_FAQ_ITEM_IDS
		)
