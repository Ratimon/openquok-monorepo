...
<Breadcrumb.Root class={cn('max-w-full', className)}>
	<Breadcrumb.List>
		...
		{#if channelLabel?.trim()}
			<Breadcrumb.Item>
				<Breadcrumb.Link href={humanizerHref} class={PUBLIC_LANDING_BREADCRUMB_LINK_CLASS}>
					Humanizer
				</Breadcrumb.Link>
			</Breadcrumb.Item>
			<Breadcrumb.Separator class={PUBLIC_LANDING_BREADCRUMB_SEPARATOR_CLASS} />
		...
		{:else}
			<Breadcrumb.Item>
				<Breadcrumb.Page class={PUBLIC_LANDING_BREADCRUMB_PAGE_CLASS}>Humanizer</Breadcrumb.Page>
			</Breadcrumb.Item>
		{/if}
	</Breadcrumb.List>
</Breadcrumb.Root>
