...
<Breadcrumb.Root class={cn('max-w-full', className)}>
	<Breadcrumb.List>
		...
		{#if channelLabel?.trim()}
			<Breadcrumb.Item>
				<Breadcrumb.Link href={bestTimeToPostHref} class={PUBLIC_LANDING_BREADCRUMB_LINK_CLASS}>
					Best Time to Post
				</Breadcrumb.Link>
			</Breadcrumb.Item>
			<Breadcrumb.Separator class={PUBLIC_LANDING_BREADCRUMB_SEPARATOR_CLASS} />
		...
		{:else}
			<Breadcrumb.Item>
				<Breadcrumb.Page class={PUBLIC_LANDING_BREADCRUMB_PAGE_CLASS}>Best Time to Post</Breadcrumb.Page>
			</Breadcrumb.Item>
		{/if}
	</Breadcrumb.List>
</Breadcrumb.Root>
