<script lang="ts">
	...
	let {
		...
		genericHref = url(route(getRootPathPublicBestTimeToPost()))
	}: Props = $props();

	const headingId = 'best-time-to-post-channel-hub-heading';
...
<section class="container mx-auto mt-12 max-w-[1600px] px-4" aria-labelledby={headingId}>
	<div class="space-y-2">
		...
		<h2 id={headingId} class="text-xl font-bold tracking-tight text-base-content sm:text-2xl">
			Best Time to Post for other channels
		</h2>
		<p class="max-w-2xl text-sm text-base-content/70">
			Each page defaults the calculator to that platform’s benchmark windows. Use the generic tool when you
			want to switch platforms without leaving the page.
		</p>
	</div>

	<ul
		class="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"
		aria-label="Best Time to Post by channel"
	>
		<li>
			<a
				href={genericHref}
			...
			>
				...
				<span class="space-y-1 text-left">
					...
					<span class="block text-xs leading-relaxed text-base-content/65">
						Pick any platform — no channel default.
					</span>
				</span>
				<span class="mt-auto text-xs font-semibold text-primary">
					{isGenericActive ? 'Current page' : 'Open calculator →'}
				</span>
			</a>
		</li>
		...
		{#each channelLinksVm as channelVm (channelVm.slug)}
			...
			<li>
				...
				>
					...
					<span class="mt-auto text-xs font-semibold text-primary">
						{isActive ? 'Current page' : 'Open calculator →'}
					</span>
				</a>
			</li>
		{/each}
