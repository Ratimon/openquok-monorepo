...
<Breadcrumb.Root class={cn('max-w-full', className)}>
	<Breadcrumb.List>
		...
		{#if channelLabel?.trim()}
			<Breadcrumb.Item>
				<Breadcrumb.Link href={payloadWizardHref} class={PUBLIC_LANDING_BREADCRUMB_LINK_CLASS}>
					Payload Wizard
				</Breadcrumb.Link>
			</Breadcrumb.Item>
			<Breadcrumb.Separator class={PUBLIC_LANDING_BREADCRUMB_SEPARATOR_CLASS} />
		...
		{:else}
			<Breadcrumb.Item>
				<Breadcrumb.Page class={PUBLIC_LANDING_BREADCRUMB_PAGE_CLASS}>Payload Wizard</Breadcrumb.Page>
			</Breadcrumb.Item>
		{/if}
	</Breadcrumb.List>
</Breadcrumb.Root>
