<script lang="ts">
	...
	const installCommand = $derived.by(() => {
		if (extensionVm.extensionType === 'mcp') return mcpInstall;
		if (extensionVm.extensionType === 'skills') return skillsInstall;
		return null;
	});

	const showBothInstallTabs = $derived(extensionVm.extensionType === 'both' && (skillsInstall || mcpInstall));
...
<div class="space-y-4 pt-4">
			...
			<Tabs.Root bind:value={installTab} class="space-y-3">
				...
				<Tabs.Content value="mcp">
					{#if mcpInstall}
						<TerminalCommandMock
							code={mcpInstall}
							ariaLabel={`MCP install for ${extensionVm.title}`}
						/>
					{:else}
	...
	{:else if installCommand}
		<div>
			<h4 class="mb-2 text-sm font-semibold text-base-content">Install</h4>
			<TerminalCommandMock code={installCommand} ariaLabel={`Install command for ${extensionVm.title}`} />
		</div>
	{/if}
