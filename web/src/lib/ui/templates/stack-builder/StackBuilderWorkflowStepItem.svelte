<script lang="ts">
	import type { StackBuilderLibraryItemKind, StackBuilderWorkflowStepViewModel } from '$lib/stack-builder/stackBuilder.types';
	...
	type Props = {
		stepVm: StackBuilderWorkflowStepViewModel;
		stepNumber: number;
		index: number;
		onUpdate: (patch: Partial<StackBuilderWorkflowStepViewModel>) => void;
		onRemove: () => void;
	...
	function stepKindLabel(kind: StackBuilderLibraryItemKind): string {
	...
	<Accordion.Root class="w-full">
		<Accordion.Item bind:open={expanded} class="border-none">
			...
			<Accordion.Content class="overflow-hidden px-3 pb-3">
				{#if stepVm.type === 'command'}
					<StackBuilderCommandStepCard stepVm={stepVm} onUpdate={onUpdate} />
				{:else}
					<StackBuilderTextStepCard stepVm={stepVm} onUpdate={onUpdate} />
				{/if}
			</Accordion.Content>
		</Accordion.Item>
	</Accordion.Root>
