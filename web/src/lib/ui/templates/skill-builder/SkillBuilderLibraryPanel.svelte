<script lang="ts">
	...
	type Props = {
		itemsVm: SkillBuilderLibraryItemViewModel[];
		onAddItem: (itemVm: SkillBuilderLibraryItemViewModel) => void;
	};

	type ExtensionLibraryGroup = {
		listingSlug: string;
		listingTitle: string;
		cliItems: SkillBuilderLibraryItemViewModel[];
		mcpItems: SkillBuilderLibraryItemViewModel[];
	};
	...
	function groupItemsByExtension(items: SkillBuilderLibraryItemViewModel[]): ExtensionLibraryGroup[] {
	...
	function kindIcon(kind: SkillBuilderLibraryItemKind): IconName {
		return kind === 'cli' ? icons.Terminal.name : icons.Bot.name;
	}

	function kindLabel(kind: SkillBuilderLibraryItemKind): string {
