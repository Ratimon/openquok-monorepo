<script lang="ts">
	...
	import { TIKTOK_BUSINESS_LANDING_KANBAN_CARDS } from './tiktokBusinessLandingKanbanMock';
	...
	const kanbanPosts = TIKTOK_BUSINESS_LANDING_KANBAN_CARDS.map((card) => ({ tagNames: card.tagNames }));
	...
	const filteredCards = $derived(
		filterKanbanCardsByReview(
			filterKanbanCardsBySource(TIKTOK_BUSINESS_LANDING_KANBAN_CARDS, sourceFilter),
			reviewFilter
		)
	);
...
<div class="bg-base-100 text-base-content">
	<div class="border-b border-base-300 px-4 py-3">
		...
		<p class="mt-1 text-xs text-base-content/70">
			Review agent and human drafts, then drag TikTok (Business) clips between Drafted and Scheduled.
		</p>
	</div>
