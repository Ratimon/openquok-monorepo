<script lang="ts">
	...
	import {
		TIKTOK_BUSINESS_LANDING_MOCK_BODY,
		TIKTOK_BUSINESS_LANDING_MOCK_CHANNEL,
		TIKTOK_BUSINESS_LANDING_MOCK_MEDIA_URLS,
		TIKTOK_BUSINESS_LANDING_MOCK_PROVIDER_SETTINGS,
		TIKTOK_BUSINESS_LANDING_MOCK_SCHEDULED_LOCAL
	} from './tiktokBusinessLandingMock';
	...
	const mockChannels = [TIKTOK_BUSINESS_LANDING_MOCK_CHANNEL];
	const selectedIds = [TIKTOK_BUSINESS_LANDING_MOCK_CHANNEL.id];
	let settingsOpen = $state(true);
	let scheduledLocal = $state(TIKTOK_BUSINESS_LANDING_MOCK_SCHEDULED_LOCAL);
	const providerSettings = TIKTOK_BUSINESS_LANDING_MOCK_PROVIDER_SETTINGS;
	...
	<div class="grid grid-cols-1 divide-y divide-base-300 lg:grid-cols-2 lg:divide-x lg:divide-y-0">
		<div class="flex flex-col gap-4 p-4">
			...
			<div class="rounded-lg border border-base-300 bg-base-100/30 p-3">
				<label
					...
					for="landing-tiktok-business-mock-body"
				>
					Post body
				</label>
				<textarea
					id="landing-tiktok-business-mock-body"
					readonly
					rows="3"
					class="textarea textarea-bordered w-full resize-none text-sm leading-relaxed"
					value={TIKTOK_BUSINESS_LANDING_MOCK_BODY}
				></textarea>
				<p class="mt-2 text-xs text-base-content/50">
					{TIKTOK_BUSINESS_LANDING_MOCK_BODY.length} / 2,200
				</p>
			</div>

			<SettingsAccordion
				bind:open={settingsOpen}
				channel={TIKTOK_BUSINESS_LANDING_MOCK_CHANNEL}
		...
		<div class="bg-base-200/20">
			...
			<div class="p-4">
				<ShowAllProviders
					channel={TIKTOK_BUSINESS_LANDING_MOCK_CHANNEL}
					previewText={TIKTOK_BUSINESS_LANDING_MOCK_BODY}
					maximumCharacters={2200}
					mediaUrls={TIKTOK_BUSINESS_LANDING_MOCK_MEDIA_URLS}
					{previewMetaLabel}
					{providerSettings}
				/>
			</div>
