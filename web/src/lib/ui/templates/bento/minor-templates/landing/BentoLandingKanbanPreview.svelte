<script lang="ts">
	...
	} from '$lib/ui/templates/bento/minor-templates/landing/landingKanbanMockTags';
	...
	const kanbanPosts = $derived(enrichedCards.map((card) => ({ tagNames: card.tagNames })));

	let allTags = $state(true);
	...
	const filteredCards = $derived(
		filterKanbanCardsByReview(
			filterKanbanCardsBySource(
				filterKanbanCardsByTags(enrichedCards, allTags, selectedTagNames),
				sourceFilter
			),
			reviewFilter
		)
	);
	const columnsVm = $derived(
		buildKanbanColumnsWithTimeFilter(filteredCards, upcomingTimeFilter, pastTimeFilter)
	);
