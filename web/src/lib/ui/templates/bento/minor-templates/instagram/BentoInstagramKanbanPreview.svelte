<script lang="ts">
	...
	const kanbanPosts = INSTAGRAM_LANDING_KANBAN_CARDS.map((card) => ({ tagNames: card.tagNames }));

	let sourceFilter = $state<PostKanbanSourceFilter>('all');
	let timeFilter = $state<PostKanbanTimeFilter>('all-upcoming');

	const filteredCards = $derived(filterKanbanCardsBySource(INSTAGRAM_LANDING_KANBAN_CARDS, sourceFilter));
...
<div class="bg-base-100 text-base-content">
	<div class="border-b border-base-300 px-4 py-3">
		...
		<p class="mt-1 text-xs text-base-content/70">
			Review agent and human drafts, then move Instagram posts between Drafted and Scheduled.
		</p>
	</div>
