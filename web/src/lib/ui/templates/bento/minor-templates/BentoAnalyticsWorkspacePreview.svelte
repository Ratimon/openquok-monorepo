<script lang="ts">
	...
	type LandingAnalyticsVm = {
		seriesVm: AnalyticsSeriesViewModel[];
		totals: string[];
	};

	type Props = {
		mockChannels: CreateSocialPostChannelViewModel[];
		mockWorkspaceId: string;
		buildAnalyticsVm: (
			dateWindowDays: number,
			filteredChannels: readonly CreateSocialPostChannelViewModel[]
		) => LandingAnalyticsVm;
	...
	const analyticsVm = $derived(buildAnalyticsVm(dateWindowDays, filteredIntegrationsVm));

	const showTrendsPanel = $derived(
		analyticsVm.seriesVm.some((series) => Boolean(series.integrationId?.trim()))
	);
	...
	<section class="space-y-3 px-4 py-4">
		...
		{#if showTrendsPanel}
			<div class="pointer-events-auto">
				<AnalyticsOverviewPanel seriesVm={analyticsVm.seriesVm} />
			</div>
		{/if}
		<RenderAnalyticsGrid
