...
const GENERIC_SKILL_BUILDER_FAQ_ITEMS: readonly PublicFaqItem[] = [
	...
	},
	{
		...
		description:
			`Stay on this page for a blank workflow, or scroll to the By channel section on ${faqLink(publicFaqHref.skillBuilderTool, 'Skill Builder')} and pick a network — Facebook, Instagram, X, and others. Each channel page opens with openquok CLI steps already filled in for that platform: typical post commands, analytics recipes, and sample payloads you can edit before you export.`
	}
];
...
function tailorSkillBuilderFaqItem(
	...
	const toolLinks = buildToolChannelFaqLinks('skill-builder', channelSlug);
	...
	switch (item.title) {
		...
		case 'What does Skill Builder do?':
			return {
				title: item.title,
				description:
					`Pick building blocks from the skill and MCP servers catalog, start from the pre-loaded ${platformLabel} workflow on ${faqLink(toolLinks.toolChannel, `${platformLabel} Skill Builder`)}, arrange CLI commands and notes, and preview a SKILL.md file you can download or save as a playbook. It is open source and free to use — no workspace required to export.`
			};
