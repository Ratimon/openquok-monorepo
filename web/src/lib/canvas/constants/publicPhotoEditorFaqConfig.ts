...
const GENERIC_PHOTO_EDITOR_FAQ_ITEMS: readonly PublicFaqItem[] = [
	{
		...
		description:
			`A free browser-based design canvas on ${faqLink(publicFaqHref.photoEditorTool, 'Photo Editor')}. Resize images for social channels, add text and elements from templates or stock photos, and download a PNG — or save to your cloud library when you are signed in.`
	},
	{
	...
	},
	{
		...
		description:
			`General presets (square, landscape, portrait) plus platform tabs for Facebook, Instagram, Threads, TikTok, YouTube, LinkedIn, and X. ${faqLink(publicFaqHref.channels, 'Channel pages')} open with that network’s first recommended preset selected.`
	},
	{
	...
	},
	{
		...
		description:
			`Stay on this page for General presets, or scroll to the By channel section on ${faqLink(publicFaqHref.photoEditorTool, 'Photo Editor')} and pick a network — Facebook, Instagram, X, and others. Each channel page opens with platform-specific aspect ratios already selected.`
	}
];
...
function tailorPhotoEditorFaqItem(
	...
	const toolLinks = buildToolChannelFaqLinks('photo-editor', channelSlug);

	switch (item.title) {
		case 'What is the OpenQuok Photo Editor?':
			return {
				title: item.title,
				description: `A free browser-based design canvas for ${platformLabel}. Resize images for ${platformLabel} feeds, stories, and covers, add text and elements, and download a PNG — or save to your cloud library when you are signed in. This ${faqLink(toolLinks.toolChannel, `${platformLabel} Photo Editor page`)} opens with ${platformLabel} aspect presets pre-selected.`
			};
