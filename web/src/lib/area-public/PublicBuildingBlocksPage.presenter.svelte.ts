...
export class PublicBuildingBlocksPagePresenter {
	...
	applyClientFilters(
		...
		return this.getListingPresenter.filterAndSortExtensions(buildingBlocks, filters, tagFilterVm);
	}

	/** Stateless — safe for `+page.server.ts` (SSR). */
	async loadBuildingBlocksHubStateless(params: {
	...
	syncHubFromLoad(params: {
		...
		this.filteredBuildingBlocksVm = this.applyClientFilters(params.hub.extensions, params.filters);
	...
	/** Update filters and recompute filtered building blocks (client-side). */
	updateFilters(overrides: Partial<ExtensionsHubFilters>): string {
		...
		if (this.hubVm) {
			this.filteredBuildingBlocksVm = this.applyClientFilters(this.hubVm.extensions, nextFilters);
		}
