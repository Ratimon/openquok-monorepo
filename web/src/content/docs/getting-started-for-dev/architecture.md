...
When <Badge text="config/orchestratorFlows.ts" variant="path" /> keeps transport on `in_process`, the API runs flows inline; with `bullmq`, these workers execute jobs from Redis queues shared with the API.
