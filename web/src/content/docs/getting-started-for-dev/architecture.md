...
Sidebar tabs and section order are declared in <Badge text="src/lib/docs/constants/config.ts" variant="path" /> as <code>docsTabs</code>: General, Cloud, Self-hosting, CLI, MCP, Public API, Contributing. Each top-level folder maps to one or more autogenerated sidebar sections. <code>/docs</code> loads the General introduction (<Badge text="getting-started/" variant="path" />) without changing the URL. Unmatched slugs also land on General — not Self-hosting.
