...
| <Badge text="-m" variant="param" /> <Badge text="--media" variant="param" /> | Comma-separated storage paths or URLs per flag (the CLI supplies a placeholder media id when you did not run upload). Values from repeated <code>-m</code> are merged into the main post's <code>media</code> list. Prefer upload JSON; for attachments on individual follow-up lines use <a href="/docs/cli-examples/threads">Threads examples</a> or <Badge text="-j" variant="param" />. |
