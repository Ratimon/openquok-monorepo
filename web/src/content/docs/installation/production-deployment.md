...
1) **Vercel**  for the **backend** (Express.js) (<Badge text="/backend" variant="path" />)

2)  **Vercel** for **web** (SvelteKit) (<Badge text="/web" variant="path" />)

3) (Optional) the **CLI auth server** (<Badge text="/agent/server" variant="path" /> — OAuth device flow) **Supabase** remains the database and auth provider for the API.

It is noted that **Orchestrator workers** (BullMQ) are **not** run on Vercel. We deploy **separate always-on processes** (for example on <a href="/docs/installation/railway">Railway</a>) that share the same **Redis** and **Supabase** credentials as the API. See <a href="/docs/configuration-worker">Configuration - Worker</a> and <a href="/docs/developer-guidelines/orchestrator-workflows">Orchestrator workflows</a>.

<Callout type="tip" title="Self-host">
<p>To run API, web, Redis, and BullMQ workers on your own host from this monorepo, follow <a href="/docs/installation/docker-compose">Docker Compose (self-host)</a>. After <code>docker compose … up --build</code>, open the UI at <code>http://localhost:4007</code> (API at <code>http://localhost:3000</code>). That path still needs an operator-provided Supabase project; see <a href="/docs/installation/system-requirements">System requirements</a>. To upgrade an existing install, see <a href="/docs/installation/docker-compose#update-to-a-newer-version">Update to a newer version</a>.</p>
</Callout>
...
- **Recommended:** managed **Redis** for production OAuth flows, BullMQ orchestration, and shared **rate-limit counters** when <Badge text="RATE_LIMIT_REDIS_ENABLED" variant="envBackend" /> is on (production default).

<Callout type="warning">
In production, OAuth connection state must be durable across instances (avoid <Badge text="CACHE_PROVIDER=memory" variant="envBackend" />)
</Callout>
...
- **Never commit** real secrets. Use <Badge text="backend/.env.development.example" variant="path" /> and <Badge text="backend/.env.production.local" variant="path" /> locally. Use the **Vercel dashboard** in production.

<Callout type="tip">
For the CLI auth server, use <Badge text="agent/server/.env.production.example" variant="envBackend" /> as a template and keep production values in <Badge text="agent/server/.env.production.local" variant="envBackend" /> — see <a href="/docs/configuration-agent">Configuration - Agent</a>.
</Callout>
...
<Callout type="note">
Align <Badge text="VITE_API_BASE_URL" variant="envWeb" /> with <Badge text="BACKEND_DOMAIN_URL" variant="envBackend" /> (same public API origin in production).
</Callout>
...
Check API startup logs for rate limiters using <code>store: redis</code> when <Badge text="RATE_LIMIT_REDIS_ENABLED" variant="envBackend" /> is enabled. If public pages return 503 (SSR maps API 429), see the runbook in <a href="/docs/configuration-backend/rate-limiting">Rate limiting</a>.
